...
ndnr.initFace = function(stringPrefix,transport, streamOrPort, ack) {
  ...
  if (transport == undefined) {
    ...
    ndnr.face.transport.connect(ndnr.face, function(){console.log('connection to daemon via tcp')})
  ...
  ndnr.face.transport.connect(ndnr.face, function(){console.log('connecting to daemon from repo')})
  console.log(stringPrefix)
  }
...
function fulfillInterest(prefix, interest, transport) {
  ...
  if (utils.endsWithSegmentNumber(interest.name)) {
    ...
    if (down != false){var db = level(path + objectStoreName, {db: down})}else{var db = level(path +objectStoreName, {valueEncoding: 'binary'})}
    db.get((requestedSegment).toString(), function(er, data) {
      console.log(er, data)
      if (er == undefined) {
    ...
    function crawl(q, lastfail) {
      ...
      console.log(path +q)
      if (down != false){cursor = level(path + q ,{db:down})}else{cursor = level(path + q + '/')}
        ...
        if ((interest.exclude == null) || (!interest.exclude.matches(new ndn.Name.Component(data.key)))) {
          ...
          if (data.key == "%00") {
            ...
            if ((interest.minSuffixComponents == null) || (suffixIndex >= interest.minSuffixComponents )) {
              ...
              if (down != false){var thisdb = level(data.value, {db: down})}else{var thisdb = level(data.value, {valueEncoding: 'binary'})}
...
function recursiveSegmentRequest(face, prefix, objectStoreName) {
  ...
  function putContentArray () {
    ...
    function putSegment(seg) {
      ...
      console.log(encoded)
      var encoded = contentArray[seg].wireEncode()
      if (down != false) {
        db.put(seg.toString(), encoded.toHex(), function(err, dar){ console.log('put callback', err, dar);db.close()})
      } else {

        db.put(seg.toString(), encoded.buffer, function(err, dar){ console.log('put callback', err, dar);db.close()})


      }

      console.log('put seg ', seg, db)
      contentArray.pop()
...
function buildObjectStoreTree(prefix, objectStoreName, onFinished, arg) {
  ...
  for (var i = 1; i < comps.length; i++ ) {
    console.log(comps)
    var value = path + comps.slice(0, i +1 ).join('/')
    var keyComp = comps[i]

    var slevel = path + '/'  + comps.slice(1,i ).join('/')

    console.log('!!!!!!!!!!!!!!!!!!!!!',path, slevel, keyComp, value)
    if (down != false){var db = level(slevel, {db: down})}else{var db = level(slevel)}
    db.put(keyComp, value, function(er){console.log(er);db.close()})
