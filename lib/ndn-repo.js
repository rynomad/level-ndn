...
if (process && process.env){
  var path = process.env.HOME || process.env.USERPROFILE || process.env.HOMEPATH
  path += "/.level-ndn"
  console.log(path)
}else{
  var path = ".ndn-repo/"
...
function fulfillInterest(prefix, interest, transport) {
  ...
  if (utils.endsWithSegmentNumber(interest.name)) {
    ...
    if (down != false){var db = level(path + objectStoreName, {db: down, valueEncoding: 'binary'})}else{var db = level(path +objectStoreName, {valueEncoding: 'binary'})}
        ...
        if ((interest.exclude == null) || (!interest.exclude.matches(new ndn.Name.Component(data.key)))) {
          ...
          if (data.key == "%00") {
            ...
            if ((interest.minSuffixComponents == null) || (suffixIndex >= interest.minSuffixComponents )) {
              ...
              if (down != false){var thisdb = level(data.value, {db: down, valueEncoding: 'binary'})}else{var thisdb = level(data.value, {valueEncoding: 'binary'})}
