...
var ndnr = {};
if (down != false){ndnr.db = subl(level(path ,{db:down}))}else{ndnr.db = subl(level(path ))}
          ...
          if ((interest.exclude == null) || (!interest.exclude.matches(new ndn.Name.Component(data.key)))) {
            ...
            if (data.key == "%00") {
              ...
              if ((interest.minSuffixComponents == null) || (suffixIndex >= interest.minSuffixComponents )) {
                ...
                ndnr.db.sublevel(data.value).get('0', {valueEncoding: 'binary'}, function(err, data){
                  console.log("got to data in sublevel",data)
                  if (interest.publisherPublicKeyDigest != undefined) {
                    var d = new ndn.Data()
                    d.decode(data)
                    if (ndn.DataUtils.arraysEqual(d.signedInfo.publisher.publisherPublicKeyDigest, interest.publisherPublicKeyDigest.publisherPublicKeyDigest)) {
  ...
  function putContentArray () {
    ...
    for (var i = 0; i < contentArray.length; i++){
      if (down != false) {
        ...
        op = {type: 'put', key: i.toString(), value: contentArray[i].wireEncode().buffer, valueEncoding: 'binary'}
      }
      ops.push(op)
    }

    ndnr.db.sublevel(objectStoreName).batch(ops, function(err){console.log('segments put in db');})
...
function buildObjectStoreTree(prefix, objectStoreName, onFinished, arg) {
  ...
  ndnr.db.batch(ops, function(err){
    console.log("made objectStoreTree", ndnr.db.sublevels['/test']);
    if (arg != undefined) {
