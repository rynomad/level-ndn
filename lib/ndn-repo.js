...
function fulfillInterest(prefix, interest, transport) {
  ...
  if (utils.endsWithSegmentNumber(interest.name)) {
    ...
    if (down != false){var db = level(path + objectStoreName, {db: down})}else{var db = level(path +objectStoreName)}
        ...
        if ((interest.exclude == null) || (!interest.exclude.matches(new ndn.Name.Component(data.key)))) {
          ...
          if (data.key == "%00") {
            ...
            if ((interest.minSuffixComponents == null) || (suffixIndex >= interest.minSuffixComponents )) {
              ...
              if (down != false){var thisdb = level(path + data.value, {db: down})}else{var thisdb = level(path + data.value)}
...
function buildObjectStoreTree(prefix, objectStoreName, onFinished, arg) {
  ...
  for (var i = comps.length - 1; i > 0; i-- ) {
    console.log(comps, down)
    var value = comps.join('/')
    var keyComp = comps.pop()

    var slevel = comps.join('/') || '/'
    if (down != false){var db = level("~/.ndn-repo" +slevel, {db: down})}else{var db = level("~/.ndn-repo" +slevel)}
    db.put(keyComp, value, function(){db.close()})

  }
  if (arg != undefined) {
    ndn.Face.registeredPrefixTable.splice(0,1)
